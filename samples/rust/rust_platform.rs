...
use kernel::{
    file::File, file_operations::FileOperations, io_buffer, io_mem, miscdev, mm,
    module_platform_driver, of, platform, prelude::*, sync::Ref, sync::RefBorrow,
    sync::RevocableRwSemaphore, sync::UniqueRef,
};
...
type DeviceData = kernel::sync::RevocableRwSemaphore<Data>;

struct Driver;
impl FileOperations for Driver {
    type Wrapper = Ref<DeviceData>;
    ...
    fn read(
        ...
        let guard = this.try_lock().ok_or(Error::ENXIO)?;
    ...
    fn mmap(data: RefBorrow<'_, DeviceData>, _file: &File, vma: &mut mm::virt::Area) -> Result {
        ...
        let guard = data.try_lock().ok_or(Error::ENXIO)?;
...
impl platform::Driver for Driver {
    ...
    fn probe(dev: &mut platform::Device, _id_info: Option<&Self::IdInfo>) -> Result<Self::Data> {
        ...
        // SAFETY: `revocable_rwsemaphore_init` is called below.
        ...
        kernel::revocable_rwsemaphore_init!(data.as_mut(), "OpenDice::Data");

        let data = Ref::<DeviceData>::from(data);

        let mut guard = data.try_lock().ok_or(Error::ENXIO)?;
